...
/**
 * update-wa.js — unified supunbail maintenance script
 *
 * Usage:
...
const supunbail_VERSION_PATH = join(ROOT_DIR, 'lib', 'Defaults', 'supunbail-version.json');
...
function installAcorn() {
    const depsRoot = mkdtempSync(join(tmpdir(), 'supunbail-acorn-'));
...
function syncPerModuleFiles(bundleContent, waVersion) {
    if (!waVersion) {
        ...
        if (m) {
            waVersion = m[1];
        } else if (existsSync(supunbail_VERSION_PATH)) {
            waVersion = JSON.parse(readFileSync(supunbail_VERSION_PATH, 'utf8')).version.join('.');
        } else {
            waVersion = 'unknown';
        }
    }

    writeFileSync(supunbail_VERSION_PATH, JSON.stringify({ version: waVersion.split('.').map(Number) }) + '\n', 'utf8');
...
async function updateVersion() {
    ...
    writeFileSync(supunbail_VERSION_PATH, JSON.stringify({ version }) + '\n', 'utf8');
    console.log(`[version] ✓ Updated lib/Defaults/supunbail-version.json`);
