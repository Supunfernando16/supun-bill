...
const useSqliteAuthState = async (databaseFilePath) => {
    ...
    db.exec(`
CREATE TABLE IF NOT EXISTS supunbail_auth_kv (
  k TEXT PRIMARY KEY NOT NULL,
  v TEXT NOT NULL
)`);
    const sel = db.prepare('SELECT v FROM supunbail_auth_kv WHERE k = ?');
    const upsert = db.prepare('INSERT OR REPLACE INTO supunbail_auth_kv (k, v) VALUES (?, ?)');
    const del = db.prepare('DELETE FROM supunbail_auth_kv WHERE k = ?');
