...
const extractDeviceJids = (result, myJid, excludeZeroDevices) => {
    ...
    for (const userResult of result) {
        ...
        const decodedId = (0, WABinary_1.jidDecode)(id) || {};
        const user = decodedId.user;
        const server = decodedId.server || 's.whatsapp.net';
            ...
            for (const { id: device, keyIndex } of deviceList) {
                if ((!excludeZeroDevices || device !== 0) && 
                    ...
                    extracted.push({
                        ...
                        device,
                        server,
                        jid: (0, WABinary_1.jidEncode)(user, server, device)
                    });
                }
            }
        }
