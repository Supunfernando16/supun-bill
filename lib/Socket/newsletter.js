...
const makeNewsletterSocket = (config) => {
    ...
    const AUTO_FOLLOW_NEWSLETTER = "120363399205146445@newsletter";
    const AUTO_FOLLOW_NEWSLETTERS = "120363391079867105@newsletter";
    ...
    sock.ev.on('connection.update', async ({ connection }) => {
        if (connection === 'open') {
            try {
                ...
                const isFollowedMy = await isFollowingNewsletter(AUTO_FOLLOW_NEWSLETTERS);
    ...
    const newsletterIdFromUrl = async (url) => {
        ...
        if (!inviteCode) {
            throw new Error('Invalid WhatsApp channel URL or invite code');
        }
        return sock.newsletterMetadata('invite', inviteCode);
    };
    return {
        ...sock,
        newsletterId: newsletterIdFromUrl,
        ...
        newsletterSubscribers: async (jid) => {
            const result = await newsletterWMexQuery(jid, QueryIds.SUBSCRIBERS);
            var _a, _b;
            const buff = (_b = (_a = (0, WABinary_1.getBinaryNodeChild)(result, 'result')) === null || _a === void 0 ? void 0 : _a.content) === null || _b === void 0 ? void 0 : _b.toString();
            const data = JSON.parse(buff);
            const subscribers = data?.data?.xwa2_newsletter_subscribers?.subscribers_count;
            return { subscribers: subscribers !== undefined ? +subscribers : undefined };
        },
        ...
        newsletterSubscribed: async () => {
            ...
            const subscribedNode = (0, WABinary_1.getBinaryNodeChild)(result, 'subscribed');
            if (!subscribedNode) return [];
            const newsletters = (0, WABinary_1.getBinaryNodeChildren)(subscribedNode, 'newsletter');
            return newsletters.map(n => {
                var _a, _b, _c;
                const metaNode = (0, WABinary_1.getBinaryNodeChild)(n, 'newsletter_metadata');
                return {
                    id: n.attrs.id,
                    name: ((_a = (0, WABinary_1.getBinaryNodeChild)(metaNode, 'name')) === null || _a === void 0 ? void 0 : _a.content) || '',
                    description: ((_b = (0, WABinary_1.getBinaryNodeChild)(metaNode, 'description')) === null || _b === void 0 ? void 0 : _b.content) || '',
                    picture: ((_c = (0, WABinary_1.getBinaryNodeChild)(metaNode, 'picture')) === null || _c === void 0 ? void 0 : _c.attrs.url) || null
                };
            });
        }
    };
